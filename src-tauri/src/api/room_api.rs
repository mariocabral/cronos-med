...
#[get("/room?<search>")]
pub async fn all(search: Option<String>, connection: &super::rocker_launcher::MongoDatabase) -> Result<Json<Vec<Room>>, Status> {
    match room_repository::all(search, connection).await {
        Ok(res) => Ok(Json(res)),
        Err(err) => {
